...
async function followPlaceOrder(od,traderBalance,trader,userId){
  ...
  async function binance(){
    ...
    clientList = [];
    const users = await admin.firestore().collection("traders").where("api_key","!=","").get();
    if (users.docs.length>0) {
      users.docs.forEach(async (doc) =>{
    ...
    wsClient.on("formattedMessage", async (data) => {
      ...
      //   if(data.order.symbol==element.symbol &&element.balance!=0){
      //     console.log("yayyyyyyyyyyyyyyyyyyyyyyyyyyyyyyy");
      //     console.log(element);
      ...
      //   }
      //   else{
      //     console.log("position elseeeeeeeeeeeeeeeee");
      //     console.log(element);
      //   }
      // }
